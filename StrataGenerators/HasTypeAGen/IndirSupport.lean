...
/-- Membership in the support of `List.mapM f l` at `SPMF`. The support holds `args` exactly
...
theorem mem_mapM_iff' (f : LMonoTy → SPMF LExpr')
...
theorem forall₂_forall_of_cond {P : LExpr' → Prop} {Q : LMonoTy → Prop}
    ...
    (hArg : ∀ σ, Q σ → ∀ a, a ∈ SPMF.support (genArg σ) → P a)
    {args : List LExpr'} {argTys : List LMonoTy}
    (hQ : ∀ σ ∈ argTys, Q σ)
    (h : List.Rel₂ (fun arg σ => arg ∈ SPMF.support (genArg σ)) args argTys) :
    ∀ a ∈ args, P a := by
  induction h with
...
theorem mapM_forall {P : LExpr' → Prop}
    (genArg : LMonoTy → SPMF LExpr')
    (hArg : ∀ σ a, a ∈ SPMF.support (genArg σ) → P a)
    (argTys : List LMonoTy) (args : List LExpr')
...
theorem genIndir_shape (octx : OpCtx) (τ : LMonoTy)
    ...
    ∃ (name : String) (argTys : List LMonoTy) (args : List LExpr'),
      ...
      List.Rel₂ (fun arg σ => arg ∈ SPMF.support (genArg σ)) args argTys ∧
      e = mkApps (.op () ⟨name, ()⟩
...
theorem genIndirPolyCore_shape (fctx : FVarCtx) (octx : OpCtx) (pctx : PolyOpCtx)
    ...
    (he : e ∈ SPMF.support
      (genIndirPolyCore (G := SPMF) fctx octx pctx bctx τ genArg fallback maxNumArgs)) :
    (∃ (sampledTys : List LMonoTy) (name : String)
       ...
       List.Rel₂ (fun arg σ => arg ∈ SPMF.support (genArg σ)) args concreteArgTys ∧
       e = mkApps (.op () ⟨name, ()⟩
         (some (concreteArgTys.foldr (fun σ acc => LMonoTy.arrow σ acc) τ))) args)
    ∨ e ∈ SPMF.support fallback := by
...
theorem genIndirPolyCore_hasType (fctx : FVarCtx) (octx : OpCtx) (pctx : PolyOpCtx)
    ...
    (maxNumArgs : Nat)
    (hArg : ∀ σ a, a ∈ SPMF.support (genArg σ) → HasTypeA' bctx a σ)
    (hFallback : ∀ a, a ∈ SPMF.support fallback → HasTypeA' bctx a τ)
    (e : LExpr')
    (he : e ∈ SPMF.support
      (genIndirPolyCore (G := SPMF) fctx octx pctx bctx τ genArg fallback maxNumArgs)) :
    HasTypeA' bctx e τ := by
...
theorem genIndir_getVars_subset (octx : OpCtx) (τ : LMonoTy)
    (keys : List (Lambda.Identifier Unit))
    (genArg : LMonoTy → SPMF LExpr')
    (hArg : ∀ σ a, a ∈ SPMF.support (genArg σ) → LExpr.LExpr.getVars a ⊆ keys)
    (h : (findOpsInCtx octx τ).length > 0) (e : LExpr')
    (he : e ∈ SPMF.support (genIndir (G := SPMF) octx τ genArg h)) :
...
theorem genIndirPolyCore_getVars_subset (fctx : FVarCtx) (octx : OpCtx) (pctx : PolyOpCtx)
    ...
    (maxNumArgs : Nat)
    (hArg : ∀ σ a, a ∈ SPMF.support (genArg σ) → LExpr.LExpr.getVars a ⊆ keys)
    (hFallback : ∀ a, a ∈ SPMF.support fallback → LExpr.LExpr.getVars a ⊆ keys)
    (e : LExpr')
    (he : e ∈ SPMF.support
      (genIndirPolyCore (G := SPMF) fctx octx pctx bctx τ genArg fallback maxNumArgs)) :
    LExpr.LExpr.getVars e ⊆ keys := by
...
theorem genIndir_measure_le {m : LExpr' → Nat} {tvars : List TyIdentifier}
    ...
    (genArg : LMonoTy → SPMF LExpr') (d : Nat)
    (hSimple : ∀ (name : String) (argTys : List LMonoTy),
      (name, argTys) ∈ findOpsInCtx octx τ → ∀ σ ∈ argTys, ∃ k, σ ∈ SPMF.support (genLMonoTy (G := SPMF) tvars k))
    (hArg : ∀ σ, (∃ k, σ ∈ SPMF.support (genLMonoTy (G := SPMF) tvars k)) → ∀ a, a ∈ SPMF.support (genArg σ) → m a ≤ d)
...
theorem genIndirPolyCore_measure_le {m : LExpr' → Nat} {tvars : List TyIdentifier}
    ...
    (hArg : ∀ σ, (∃ k, σ ∈ SPMF.support (genLMonoTy (G := SPMF) tvars k)) → ∀ a, a ∈ SPMF.support (genArg σ) → m a ≤ d)
    (hFallback : ∀ a, a ∈ SPMF.support fallback → m a ≤ dfb)
    (hspine : ∀ (nm : String) (annot : LMonoTy) (args : List LExpr'),
      (∀ a ∈ args, m a ≤ d) →
      m (mkApps (.op () ⟨nm, ()⟩ (some annot)) args) ≤ d + args.length)
    (e : LExpr')
