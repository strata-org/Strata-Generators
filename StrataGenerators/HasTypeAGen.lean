...
/-- Soundness of `genIndirPoly`: every generated expression is well-typed.
    The IndirPoly rule generates `.op` nodes with the correct annotated type
    (`some fullArrowTy` where `fullArrowTy = concreteArgTys.foldr arrow τ`),
    so `HasTypeA.op` applies to give the op the full arrow type, and then
    `mkApps_hasType` with recursively-typed arguments gives the result type `τ`.

    The proof structure mirrors `genLExpr_sound` for the monomorphic Indir rule:
    1. The op node carries annotation `some fullArrowTy` → typed by `HasTypeA.op`
    2. Each argument is generated by `genLExprBase` at the correct concrete type
       → typed by `genLExprBase_sound`
    3. `mkApps_hasType` combines these into the final `HasTypeA' bctx e τ` -/
...
theorem genLExpr_sound (fctx : FVarCtx) (octx : OpCtx) (pctx : PolyOpCtx)
  ...
  · -- IndirPoly path (polymorphic operators)
    exact genIndirPoly_sound fctx octx pctx tvars bctx depth τ hτ hSimpleOps e he
