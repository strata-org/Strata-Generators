...
`StmtHasTypeA P C Γ s C' Γ'` is a **5-place** relation. Read it as:

> under program `P`, starting in ambient context `C` (built-in/declared functions
> and known types) and type-scope `Γ` (variable types), statement `s` is well-typed
> and *produces output contexts* `C'` and `Γ'`.

The input contexts are `(C, Γ)`; the output contexts are `(C', Γ')`. When this note
writes `C Γ ⟶ C Γ`, it means the instance where the **output** equals the **input**
— the statement leaves the contexts unchanged.
...
| ite_det : ∀ C Γ C_t Γ_t C_e Γ_e cond thenb elseb md,
    ...
    StmtHasType' τ P C Γ (.ite (.det cond) thenb elseb md) C Γ   -- output C Γ

| loop : ∀ C Γ C_body Γ_body guard measure invariants body md,
    … → StmtsHasType' τ P C Γ body C_body Γ_body →
    StmtHasType' τ P C Γ (.loop guard measure invariants body md) C Γ  -- output C Γ
```
...
```lean
| funcDecl : … → StmtHasType' τ P C Γ (.funcDecl decl md) (C.addFactoryFunction func) Γ
--                                     output C is EXTENDED ^^^^^^^^^^^^^^^^^^^^^^^^^^
| typeDecl : … C.addKnownTypeWithError … = .ok C' →
             StmtHasType' τ P C Γ (.typeDecl tc md) C' Γ            -- output C' ≠ C
```
...
```
given  StmtHasTypeA P C Γ (block label body md) C Γ
find   r ∈ support (genStmt …)  with  r realizing  StmtHasTypeA P C Γ r.stmt C Γ
```

The judgment `StmtHasTypeA P C Γ · C Γ` says **nothing about the body** — only
"input `C Γ`, output `C Γ`." And there is a *much simpler* statement with the
**identical** judgment: `exit`.

```lean
| exit : ∀ C Γ label md, StmtHasType' τ P C Γ (.exit label md) C Γ  -- also C Γ ⟶ C Γ
```

So the "completeness" obligation for a `block` could be discharged by the generator
producing an **`exit`**: the theorem would be *satisfied without the generator ever
producing a block*, let alone the intended body. Technically true, completely
useless — it never forces coverage of the nesting constructors. The typing relation,
by design, has thrown away exactly the structural information (which body, which
branches, which guard) that a *generator* completeness theorem must nail down.
...
2. **`genStmt_sound`: in support ⇒ `StmtHasTypeA`.** Everything in the support is
   well-typed.
...
constructors — an `exit`, which has the same judgment, would satisfy it. We instead
