...
for random well-typed `LExpr`s. The generator (`genLExpr`) can be found in [`HasTypeAGen/Core.lean`](../StrataGenerators/HasTypeAGen/Core.lean), and its correctness proofs in `HasTypeAGen.lean`.
...
The initial generator synthesized by Claude, while correct, generates trivial terms (constants) most of the time. 
...
```lean
-- Branch of generator responsible for generating an expression of type Bool for some non-empty size
pick
  (return bool constant)   -- 50% chance -> trivial
  (pick
    (ite ...)              -- 25% chance -> recurses to generate sub-terms
    (pick
      (eq ...)             -- 12.5%
      (pick
        (app ...)          -- 6.25%
