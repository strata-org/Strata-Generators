...
- **The symbolic evaluator silently drops every proof obligation after a second `if *`, which is unsound.** Tracked as repo issue #113, with a self-contained runnable reproducer. Reproducer and analysis in [`docs/strata-symbolic-eval-nondet-collision.md`](docs/strata-symbolic-eval-nondet-collision.md). This one is in the *evaluator*, not in any of the eight passes, and it was found by the obligation-preservation properties for the three loop passes: `NondetElim` removes every `if *`, so running it made obligations **reappear**, and chasing the growth led back to `Core.Statement.evalOneStmt`.
