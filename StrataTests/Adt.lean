...
def adtBlockChecks : List TestDecl :=
  family GenAdtBlock
    [ -- The screen that keeps an ill-typed program away from each property that uses a solver. It is also a
      -- claim of its own: an equality can use a derived constructor at its ground instance. It holds for each
      -- block that Strata accepts.
